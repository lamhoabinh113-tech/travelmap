...
<div class="dashboard-container">
    ...
    <div class="sidebar animate-fade-in">
        <div class="user-profile">
            <div class="avatar-placeholder" onclick="openAvatarUploader()">
                <?php if (!empty($_SESSION['avatar'])): ?>
                    <img src="<?php echo htmlspecialchars($_SESSION['avatar']); ?>" alt="Avatar" class="avatar-logo">
                <?php else: ?>
                    <img src="https://cdn-icons-png.flaticon.com/512/4140/4140044.png" alt="AI icon" class="avatar-logo">
                <?php endif; ?>
            </div>
            <div class="flex-grow-1">
                <h6 class="mb-0 fw-bold"><?php echo $_SESSION['full_name']; ?></h6>
                <small class="opacity-75">@<?php echo $_SESSION['username']; ?></small>
                <div class="profile-badges">
                    <span><i class="bi bi-award-fill"></i> Explorer Lv.<?php echo $explorer_level; ?></span>
                    <span><i class="bi bi-camera-fill"></i> <?php echo $photo_count; ?> anh</span>
                </div>
            </div>
            <a href="index.php?url=auth/logout" class="btn btn-white-glass btn-sm rounded-pill px-3">
                <i class="bi bi-box-arrow-right"></i>
            </a>
        </div>
        <div class="journey-stats">
            <div class="stat-card">
                <div class="stat-value"><?php echo $journey_count; ?></div>
                <small><i class="bi bi-geo-alt-fill me-1 text-primary"></i>Chuyến đi</small>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo $photo_count; ?></div>
                <small><i class="bi bi-images me-1 text-danger"></i>Ảnh đã đăng</small>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo count(array_unique($place_names)); ?></div>
                <small><i class="bi bi-map-fill me-1 text-success"></i>Địa điểm</small>
            </div>
            <div class="stat-card">
                <div class="stat-value"><?php echo number_format($total_distance, 0); ?> km</div>
                <small><i class="bi bi-signpost-2-fill me-1 text-info"></i>Quãng đường</small>
            </div>
        </div>
        <?php if (isset($_GET['avatar_success'])): ?>
            <div class="alert alert-success border-0 rounded-4 py-2 small mb-3 animate-fade-in">
                <i class="bi bi-check-circle-fill me-2"></i> Ảnh đại diện đã được cập nhật.
            </div>
        <?php elseif (isset($_GET['avatar_error'])): ?>
            <div class="alert alert-danger border-0 rounded-4 py-2 small mb-3 animate-fade-in">
                <i class="bi bi-x-circle-fill me-2"></i> <?php echo htmlspecialchars($_GET['avatar_error']); ?>
            </div>
        <?php endif; ?>
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <?php if(isset($is_friend_view) && $is_friend_view): ?>
                <div>
                    <a href="index.php?url=location/dashboard" class="text-decoration-none small fw-bold text-primary mb-1 d-block">
                        <i class="bi bi-arrow-left"></i> Quay lại của tôi
                    </a>
                    <h5 class="fw-bold mb-0">Hành Trình của <?php echo $friend_info['full_name']; ?></h5>
                </div>
            <?php else: ?>
                <div>
                    <h5 class="fw-bold mb-0">Hành Trình</h5>
                    <div class="d-flex flex-column gap-2">
                        <a href="index.php?url=feed" class="text-decoration-none small fw-bold text-primary animate-fade-in">
                            <i class="bi bi-people-fill me-1"></i> Xem bảng tin bạn bè
                        </a>
                    </div>
                </div>
        ...
        <div class="memory-list journey-timeline">
            <?php if(isset($_GET['update_success'])): ?>
                <div class="alert alert-success border-0 rounded-4 py-2 small mb-3 animate-fade-in">
                    <i class="bi bi-check-circle-fill me-2"></i> Cập nhật thành công!
                </div>
            <?php endif; ?>

            <!-- Hành trình của bạn bè (Được đẩy lên trên) -->
            <?php if(!isset($is_friend_view) && !empty($friend_locations)): ?>
                <div class="mb-4">
                    ...
                    <?php 
                    // Chỉ hiển thị tối đa 3 hành trình mới nhất từ bạn bè để không bị quá dài
                    $top_friends = array_slice($friend_locations, 0, 3);
                    foreach($top_friends as $floc): 
                    ?>
                        <div class="memory-item border-primary-subtle" style="background: #f0f7ff;" onclick="focusMemory(<?php echo (int)$floc['id']; ?>)">
                    ...
                    <?php endforeach; ?>
                    <a href="index.php?url=feed" class="btn btn-light w-100 rounded-pill py-1 small fw-bold text-primary mb-4" style="font-size: 11px;">
                        Xem tất cả bảng tin <i class="bi bi-arrow-right ms-1"></i>
                    </a>
                </div>
            ...
            <h6 class="small fw-bold text-muted mb-3 d-flex align-items-center">
                <i class="bi bi-signpost-split-fill me-2 text-primary"></i> TIMELINE HÀNH TRÌNH
            </h6>
                ...
                <div class="memory-item" onclick="focusMap(<?php echo $loc['latitude']; ?>, <?php echo $loc['longitude']; ?>, true)">
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <div class="d-flex gap-2">
                            <a href="javascript:void(0)" 
                               class="text-primary opacity-50 hover-opacity-100" 
                               onclick='event.stopPropagation(); openEditModal(<?php echo json_encode($loc); ?>)'>
                                <i class="bi bi-pencil-square"></i>
                            </a>
                            <a href="index.php?url=location/delete&id=<?php echo $loc['id']; ?>" 
                               class="text-danger opacity-50 hover-opacity-100" 
                               onclick="event.stopPropagation(); return confirm('Xóa kỷ niệm này?')">
                    ...
                    <div class="memory-meta-grid">
                        <span class="memory-chip">
                            <i class="bi bi-calendar3 text-primary"></i> <?php echo date('d/m/Y', strtotime($loc['visit_date'])); ?>
                        </span>
                        <span class="memory-chip">
                            <i class="bi bi-emoji-smile text-warning"></i> <?php echo htmlspecialchars($loc['feeling']); ?>
                        </span>
                        <span class="memory-chip">
                            <i class="bi bi-cloud-sun text-info"></i> Trời đẹp
                        </span>
                        <span class="memory-chip">
                            <i class="bi bi-images text-danger"></i> <?php echo !empty($loc['image']) ? '1+ ảnh' : '0 ảnh'; ?>
                        </span>
                    </div>
    ...
    <!-- Map -->
    <div id="map"></div>
    <div class="map-mode-pill" aria-label="Map tools">
        ...
        <button type="button" onclick="fitJourneyRoute()" title="Xem toàn bộ hành trình"><i class="bi bi-signpost-split-fill"></i></button>
    </div>
    ...
    <!-- Social Sidebar -->
    <div class="social-sidebar animate-fade-in">
        <h5 class="fw-bold mb-4">Bạn Bè</h5>

        <?php if(isset($_GET['friend_success'])): ?>
            <div class="alert alert-success border-0 rounded-4 py-2 small mb-3 animate-fade-in">Kết bạn thành công!</div>
        <?php endif; ?>
        <?php if(isset($_GET['friend_error'])): ?>
            <div class="alert alert-danger border-0 rounded-4 py-2 small mb-3 animate-fade-in">
                <?php 
                    if($_GET['friend_error'] == 'self') echo "Bạn không thể tự kết bạn với chính mình!";
                    else if($_GET['friend_error'] == 'exists') echo "Hai người đã là bạn bè rồi!";
                    else echo "Liên kết không hợp lệ hoặc có lỗi xảy ra!";
                ?>
            </div>
        <?php endif; ?>
        ...
        <div class="mb-4">
            ...
            <div class="p-3 border rounded-4 text-center bg-light">
                ...
                <small class="text-primary fw-bold">Quét để kết bạn</small>
            </div>
        </div>
        ...
        <div class="friend-list">
            <?php if(empty($friends)): ?>
                <div class="text-center py-4 opacity-50 small">
                    Chưa có bạn bè nào.<br>Hãy mời bạn bè tham gia!
                </div>
            <?php else: ?>
                <?php foreach($friends as $friend): ?>
                    <div class="friend-card">
                        <div class="avatar-placeholder" style="width: 35px; height: 35px; font-size: 14px;">
                        ...
                        <a href="index.php?url=location/friend_map&id=<?php echo $friend['id']; ?>" class="btn btn-primary btn-sm rounded-circle p-1" style="width: 25px; height: 25px; display: flex; align-items: center; justify-content: center;">
...
<?php if(!isset($is_friend_view)): ?>
<div class="camera-fab" onclick="openLocketCamera()">
    <i class="bi bi-camera-fill"></i>
</div>
<?php endif; ?>

<!-- AI Chat Widget Toggle -->
<button class="chat-toggle-btn" onclick="toggleChatWidget()" title="Travel Memory AI">
...
<div class="chat-widget" id="aiChatWidget">
    <div class="chat-widget-header">
        <div>
            <h5><i class="bi bi-globe-asia-australia me-1"></i> Travel Memory AI</h5>
            <small>Người đồng hành hành trình của bạn</small>
        </div>
        <button class="chat-close" onclick="closeChatWidget()"><i class="bi bi-x"></i></button>
    </div>
    <div class="chat-widget-body">
        <div class="chat-history" id="chatHistory">
            <div class="chat-message assistant">
                <div class="avatar">AI</div>
                <div class="bubble">Chào bạn! Hỏi tự nhiên nhé — ví dụ: "Lịch trình 2 ngày Hải Dương", "Ăn gì ngon ở Đà Nẵng", "Thời tiết Hà Nội tuần này", "Địa điểm nào đẹp gần đây". Mình dùng GPS + hành trình của bạn để tư vấn cụ thể hơn.</div>
            </div>
        </div>

        <div class="chat-quick-actions">
            <button type="button" onclick="setChatPrompt('Lịch trình 3 ngày 2 đêm đi Đà Nẵng, có check-in, quán ăn và cafe')">Lịch trình</button>
            <button type="button" onclick="setChatPrompt('Dự báo thời tiết Hải Dương 5 ngày tới, nên mang gì?')">Thời tiết</button>
            <button type="button" onclick="setChatPrompt('Địa điểm nào đẹp ở Hải Dương nên ghé?')">Địa điểm đẹp</button>
            <button type="button" onclick="setChatPrompt('Ăn gì ngon ở Đà Lạt? Gợi ý quán đặc sản')">Món ngon</button>
        </div>
    </div>
    <div class="chat-input-area">
        <textarea id="chatInput" placeholder="Nhập yêu cầu AI..." rows="4"></textarea>
        <div class="chat-input-actions">
            <small>Lịch trình · thời tiết · địa điểm · món ngon · caption.</small>
            <button type="button" class="btn-send" onclick="sendChatMessage()"><i class="bi bi-send-fill me-1"></i>Gửi</button>
        </div>
    </div>
</div>

<!-- Locket Camera Fullscreen UI -->
<div class="locket-camera-overlay" id="locketCameraUI">
    <div class="camera-header">
        <button class="camera-close" onclick="closeLocketCamera()">
            <i class="bi bi-x-lg"></i>
        </button>
        <div class="badge bg-dark bg-opacity-50 px-3 py-2 rounded-pill fw-normal shadow" id="camLocationStatus">
            <i class="bi bi-geo-alt-fill text-danger me-1"></i> Đang tìm vị trí...
        </div>
        <button class="camera-close" onclick="switchCamera()">
    ...
    <div class="camera-viewfinder">
        ...
        <div id="cameraError">
            <i class="bi bi-camera-video-off" style="font-size: 48px; opacity:0.5;"></i>
            <p class="mt-3" id="cameraErrorMsg">Không thể kết nối camera.</p>
            <button class="btn btn-light rounded-pill px-4 mt-2" onclick="startCamera(currentFacingMode)">
    ...
    <div class="post-controls" id="postControls">
        ...
        <select id="locketPrivacy" class="caption-input mt-2" style="background: rgba(255, 255, 255, 0.15); border: none; color: white;" onchange="toggleSpecificFriends('locket')">
            <option value="friends" style="background:#000;" selected>👥 Bạn bè (Chỉ bạn bè mới thấy)</option>
            <option value="specific_friends" style="background:#000;">👥 Bạn bè cụ thể (Chọn người xem)</option>
            <option value="public" style="background:#000;">🌐 Công khai (Ai cũng xem được)</option>
            <option value="private" style="background:#000;">🔒 Chỉ mình tôi (Riêng tư)</option>
        </select>

        <div class="mb-2 text-start" id="locketSpecificFriendsContainer" style="display: none; max-height: 120px; overflow-y: auto; background: rgba(0,0,0,0.5); padding: 10px; border-radius: 12px; width: 100%; border: 1px solid rgba(255,255,255,0.1);">
            <label class="form-label small fw-bold text-primary mb-1">CHỌN BẠN BÈ ĐƯỢC XEM</label>
            <?php if(!empty($friends)): ?>
                <?php foreach($friends as $f): ?>
                    <div class="form-check mb-1">
                        <input class="form-check-input locket-friend-cb" type="checkbox" value="<?php echo $f['id']; ?>" id="locket_friend_<?php echo $f['id']; ?>">
                        <label class="form-check-label text-white small" for="locket_friend_<?php echo $f['id']; ?>">
                            <?php echo htmlspecialchars($f['full_name']); ?> (@<?php echo htmlspecialchars($f['username']); ?>)
                        </label>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="text-white-50 small">Chưa có bạn bè nào để chọn.</div>
            <?php endif; ?>
        </div>
        
        <div class="post-actions mt-2">
            <button class="btn-locket-cancel" onclick="retakePhoto()">
                <i class="bi bi-arrow-left"></i>
            </button>
            <button class="btn-locket-post" onclick="postLocketPhoto()">
                <i class="bi bi-send-fill"></i> Đăng ngay lên bản đồ
            </button>
        </div>
        <button class="btn btn-outline-light rounded-pill mt-2 py-2 fw-bold" onclick="saveToAlbumPrompt()" style="border-width: 2px;">
            <i class="bi bi-images me-2"></i> Lưu vào Album
        </button>
    </div>
</div>
